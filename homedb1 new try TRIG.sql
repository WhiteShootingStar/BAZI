...
INSERT INTO SERVICE_ONE_OPER VALUES (120 , 105 ,180);
SELECT *
...
DROP TRIGGER OPERTRIG;
DROP TRIGGER TRIGPROCED;
CREATE OR REPLACE TRIGGER TRIGPROCED
BEFORE INSERT 
ON OPERATION
FOR EACH ROW
BEGIN
 SERVICE_OPER(:new.IDOPER);
 END;

 INSERT INTO  OPERATION VALUES (180,'14:50' ,93);
 UPDATE OPERATION SET VIS=96 WHERE VIS =97;
 DELETE FROM OPERATION WHERE IDOPER =180;
 DELETE FROM SERVICE_ONE_OPER WHERE OPER=180;
 INSERT INTO SERVICE_ONE_OPER VALUES (120 , 105 ,180);
 ...
 BEGIN
 SELECT HEIGTH INTO X FROM ANIMM  WHERE AN=ANIM;
 SELECT WEIGTH INTO Y FROM ANIMM WHERE AN=ANIM;
 Z:=(X*X)/Y;
 RETURN (Z);
 END;
 SELECT FUNC(12) FROM ANIMM,ANIMAL WHERE ANIMAL.IDA=ANIMM.AN;
SELECT HEIGTH  FROM ANIMM  WHERE AN=17;