...
CREATE OR ALTER TRIGGER DOCTRIG--3
ON DOCTOR
FOR UPDATE
AS
BEGIN
...
CREATE OR ALTER TRIGGER SERVICETRIG --6
ON SERVICE
INSTEAD OF DELETE
AS
BEGIN
...
CREATE OR ALTER TRIGGER OPERTRIG --5
ON OPERATION
INSTEAD OF DELETE
AS
BEGIN
...
CREATE OR  ALTER TRIGGER OWNERTRIG--1
ON OWNER
FOR INSERT,update
AS
BEGIN
...
CREATE OR ALTER PROCEDURE OPER_SERV @IDOP INT
AS
...
CREATE OR ALTER PROCEDURE  OPER_DOC_INCR @IDOP INT
AS
SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE RECAL_PRICE @OP INT, @NEW_OP_PRICE FLOAT OUTPUT
AS
BEGIN
